...
def write_to_mongo(path_to_udfile: str, dataset_name: str, ignore_docs: bool, environment: str) -> None:
    """
    ...
    documents: List[Document] = UDReader.read_docs(path_to_udfile, dataset_name, ignore_docs)
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ignore-docs", help="Ignore document boundaries", action="store_true")
    parser.add_argument("--environment", help="Which environment to use", choices=["dev", "prod"], default="dev")


    args = parser.parse_args()

    write_to_mongo(args.input_file, args.dataset_name, args.ignore_docs, args.environment)