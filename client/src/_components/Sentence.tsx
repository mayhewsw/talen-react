...
class Sentence extends React.Component<SentProps, State>{
  ...
  showPopoverFunc(index: number){
    return this.props.isActive && !this.state.mouseIsDown && index === this.state.selected_range[1]
  }
        ...
        return (
            ...
            {this.props.sent.map((tok, index) =>
              <Token 
                ...
                set_label={(label: string) => this.props.set_label(label, this.state.selected_range[0], this.state.selected_range[1])}
              />
            )}
          </div>
        )
